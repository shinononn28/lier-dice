...
setInterval(()=>{document.querySelectorAll('.cd').forEach(el=>el.textContent=cdText())},500);
function render(){
...
function showResult(){
  ...
  w.addEventListener('click',e=>{const b=e.target.closest('button');if(!b)return;closeSheet(w);
    if(b.dataset.x==='again')showHome();if(b.dataset.x==='rematch'&&socket)socket.emit('rematch');if(b.dataset.x==='review')showReview()});
}
...
function startCpu(o){
  ...
  const eng=createGame({humans:[{id:'me',name:o.name}],total:o.n,speed:o.speed,botChat:o.botChat,emit:(to,type,data)=>{if(to==='*'||to==='me')onEvent(type,data)}});
...
function connect(){
  ...
  socket.on('lchatAll',a=>{lobbyChat=a;drawLobbyChat()});
  socket.on('lchat',m=>{lobbyChat.push(m);if(lobbyChat.length>80)lobbyChat.shift();drawLobbyChat()});
  socket.on('errorMsg',m=>{toast(m);if(!room&&!document.querySelector('.home'))showHome()});
  return socket;
}
let lobbyChat=[];
const TIMER_LABEL={turn:'手番',prep:'ラウンド準備',mitsudan:'密談',accuse:'告発'};
const fmtSec=x=>x>=60&&x%60===0?`${x/60}分`:`${x}秒`;
const BOTCHAT_OPTS=[['on','あり'],['low','推理だけ'],['off','なし']];
function lobbyInfoHTML(){
  const r=room,url=location.origin+location.pathname+'?room='+r.code;
  return `<p class="muted small">ルームコード</p><div class="codebig disp">${esc(r.code)}</div>
    <div class="row"><button data-x="copy">招待リンクをコピー</button></div>
    <p class="small muted" style="margin-top:6px;overflow-wrap:anywhere">${esc(url)}</p>
    <h3 style="margin-top:10px">参加者（${r.members.length}人）</h3><div class="members">${r.members.map(m=>`<div>${esc(m.name)}${m.host?' <span class="tag rogue">ホスト</span>':''}${m.connected?'':' <span class="small muted">切断中</span>'}</div>`).join('')}</div>
    ${r.isHost?`<fieldset><legend>卓の人数（足りない分はCPU）</legend><div class="seg" data-g="n">${[4,5,6].map(n=>`<button data-v="${n}" aria-pressed="${r.n===n}">${n}人</button>`).join('')}</div></fieldset>
      <fieldset><legend>CPUのチャット</legend><div class="seg" data-g="bc">${BOTCHAT_OPTS.map(([v,l])=>`<button data-v="${v}" aria-pressed="${r.botChat===v}">${l}</button>`).join('')}</div></fieldset>
      <fieldset><legend>制限時間</legend><div class="tmgrid">${Object.keys(TIMER_LABEL).map(k=>`<span class="small">${TIMER_LABEL[k]}</span><div class="seg" data-g="tm" data-k="${k}">${[...r.timerOpts[k]].sort((a,b)=>a-b).map(v=>`<button data-v="${v}" aria-pressed="${r.timers[k]===v}">${fmtSec(v)}</button>`).join('')}</div>`).join('')}</div></fieldset>`
      :`<p class="muted small">ホストの開始を待っています…（卓は${r.n}人・CPUのチャット：${(BOTCHAT_OPTS.find(x=>x[0]===r.botChat)||[0,'なし'])[1]}）<br>制限時間：${Object.keys(TIMER_LABEL).map(k=>`${TIMER_LABEL[k]}${fmtSec(r.timers[k])}`).join('・')}</p>`}`;
}
function drawLobbyChat(){if(!lobbyW)return;const lg=lobbyW.querySelector('.lchat');if(!lg)return;
  lg.innerHTML=lobbyChat.length?lobbyChat.map(m=>m.sys?`<div class="msg sys">${esc(m.text)}</div>`:`<div class="msg chat ${m.key===myKey?'me':''}"><b>${esc(m.name)}</b><span>${esc(m.text)}</span></div>`).join(''):'<div class="msg sys">開始までここで話せます</div>';
  lg.scrollTop=lg.scrollHeight}
function showLobby(){
  if(lobbyW&&lobbyW.isConnected){lobbyW.querySelector('.linfo').innerHTML=lobbyInfoHTML();
    lobbyW.querySelector('[data-x=start]')&&(lobbyW.querySelector('[data-x=start]').hidden=!room.isHost);return}
  lobbyW=openSheet(`<div class="linfo">${lobbyInfoHTML()}</div>
    <div class="mlog lchat" style="height:150px"></div>
    <div class="chatin"><input class="lin" placeholder="待ち時間の雑談" maxlength="100" aria-label="ロビーのチャット"><button data-x="lsend">送る</button></div>
    <div class="row end"><button data-x="leave">退出</button><button class="primary" data-x="start" ${room.isHost?'':'hidden'}>ゲーム開始</button></div>`);
  drawLobbyChat();
  const inp=lobbyW.querySelector('.lin');
  const send=()=>{const t=inp.value.trim();if(!t)return;inp.value='';socket.emit('lchat',t)};
  inp.addEventListener('keydown',e=>{if(e.key==='Enter'&&!e.isComposing)send()});
  lobbyW.addEventListener('click',e=>{const b=e.target.closest('button');if(!b)return;const seg=b.closest('.seg');
    if(seg){socket.emit('settings',seg.dataset.g==='bc'?{botChat:b.dataset.v}:seg.dataset.g==='tm'?{timer:{key:seg.dataset.k,sec:+b.dataset.v}}:{n:+b.dataset.v});return}
    if(b.dataset.x==='lsend')send();
    ...
    if(b.dataset.x==='leave'){socket.emit('leave');room=null;lobbyChat=[];closeSheet(lobbyW);lobbyW=null;history.replaceState(null,'',location.pathname);showHome()}});
}

/* ---------- home ---------- */
function showHome(){
  resetClient();mode=null;const qs=new URLSearchParams(location.search).get('room');
  const opt={n:5,s:1,bc:'on'};const name=store.get('nimai-name')||'';
  const w=openSheet(`<h1 class="disp title">二枚舌の酒場</h1><p>誰が味方かわからないまま、ダイスの嘘を読み合う。</p>
    ...
    <div class="opt"><h3>CPUとひとりで遊ぶ</h3>
      ...
      <div class="small muted">CPUのチャット</div><div class="seg" data-g="bc">${BOTCHAT_OPTS.map(([v,l])=>`<button data-v="${v}" aria-pressed="${v==='on'}">${l}</button>`).join('')}</div>
      <div class="line2"><button data-x="cpu">CPU戦をはじめる</button></div></div>
    <p class="muted small">ラウンド数は人数と同じで、全員が1回ずつ親になる。奇数人数だと詐欺師が1人混ざる。6人卓はカードが4枚配られて3枚まで使える。</p>
  ...
  w.addEventListener('click',e=>{const b=e.target.closest('button');if(!b)return;const seg=b.closest('.seg');
    if(seg){opt[seg.dataset.g]=seg.dataset.g==='bc'?b.dataset.v:+b.dataset.v;seg.querySelectorAll('button').forEach(x=>x.setAttribute('aria-pressed',x===b));return}
    if(b.dataset.x==='rules')showRules();
    if(b.dataset.x==='cpu'){const nm=getName();closeSheet(w);startCpu({n:opt.n,speed:opt.s,name:nm,botChat:opt.bc})}
